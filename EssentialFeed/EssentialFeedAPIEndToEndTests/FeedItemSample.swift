//
//  FeedItemSample.swift
//  EssentialFeedAPIEndToEndTests
//
//  Created by Lorenzo Limoli on 17/10/22.
//  Copyright © 2022 Essential Developer. All rights reserved.
//

import Foundation


enum FeedItemSample{
    private static let items: [[String: String]] = [
        [
            "id": "73A7F70C-75DA-4C2E-B5A3-EED40DC53AA6",
            "description": "Description 1",
            "location": "Location 1",
            "image": "https://url-1.com",
        ],
        [
            "id": "BA298A85-6275-48D3-8315-9C8F7C1CD109",
            "location": "Location 2",
            "image": "https://url-2.com",
        ],
        [
            "id": "5A0D45B3-8E26-4385-8C5D-213E160A5E3C",
            "description": "Description 3",
            "image": "https://url-3.com",
        ],
        [
            "id": "FF0ECFE2-2879-403F-8DBE-A83B4010B340",
            "image": "https://url-4.com",
        ],
        [
            "id": "DC97EF5E-2CC9-4905-A8AD-3C351C311001",
            "description": "Description 5",
            "location": "Location 5",
            "image": "https://url-5.com",
        ],
        [
            "id": "557D87F1-25D3-4D77-82E9-364B2ED9CB30",
            "description": "Description 6",
            "location": "Location 6",
            "image": "https://url-6.com",
        ],
        [
            "id": "A83284EF-C2DF-415D-AB73-2A9B8B04950B",
            "description": "Description 7",
            "location": "Location 7",
            "image": "https://url-7.com",
        ],
        [
            "id": "F79BD7F8-063F-46E2-8147-A67635C3BB01",
            "description": "Description 8",
            "location": "Location 8",
            "image": "https://url-8.com",
        ]
    ]
    
    static func id(at index: Int) -> UUID{
        let id = items[index]["id"]!
        return .init(uuidString: id)!
    }
    
    static func description(at index: Int) -> String?{
        guard let description = items[index]["description"] else {
            return nil
        }
        return description
    }
    
    static func location(at index: Int) -> String?{
        guard let location = items[index]["location"] else {
            return nil
        }
        return location
    }
    
    static func imageURL(at index: Int) -> URL{
        let url = items[index]["image"]!
        return URL(string: url)!
    }
}
